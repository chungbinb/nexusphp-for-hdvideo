...
if (!$row) {
	...
	if (!isset($_GET["cmtpage"])) {
		...
		$sp_torrent = get_torrent_promotion_append($row['sp_state'],'word', false, '', 0, '', $row['__ignore_global_sp_state'] ?? false, $row['id'] ?? 0);
...
//						$other_processing_info = $copy_row['processing_name'].", ";
					...
					$sp_info = get_torrent_promotion_append($copy_row['sp_state'], '', false, '', 0, '', $copy_row['__ignore_global_sp_state'] ?? false, $copy_row['id'] ?? 0);
