...
function zjh_next_active($room, $seat)
{
    ...
    for ($i=1; $i<=$seatCount; $i++) {
        $next = ((int)$seat + $i) % $seatCount;
        if (($room['players'][$next]['status'] ?? '') === 'active' && empty($room['players'][$next]['revealed'])) return $next;
    }
    return -1;
}
...
function zjh_advance(&$room, $seat)
{
    ...
    if (!$waiting) { zjh_settle($room, zjh_best_seat($room, $active), '所有仍在局玩家均已亮牌'); return; }
    if (count($waiting) === 1 && count($active) > 1) {
        $lastSeat = $waiting[0];
        $room['players'][$lastSeat]['seen'] = true;
        $room['players'][$lastSeat]['revealed'] = true;
        zjh_log($room, $room['players'][$lastSeat]['username'] . ' 成为最后一位可行动玩家，系统自动亮牌');
        zjh_settle($room, zjh_best_seat($room, $active), '所有仍在局玩家均已亮牌');
        return;
    }
    if ((int)$room['action_count'] >= ZJH_MAX_ACTIONS) {
        foreach ($waiting as $waitingSeat) {
            $room['players'][$waitingSeat]['seen'] = true;
            $room['players'][$waitingSeat]['revealed'] = true;
        }
        zjh_settle($room, zjh_best_seat($room, $active), '达到操作上限，所有在局玩家自动亮牌');
        return;
    }
...
function zjh_apply(&$room, $seat, $action, $targetSeat = null)
{
    ...
    $unit = zjh_action_cost($room['current_bet'], !empty($player['seen']), $room['base']);
    $mustAllIn = (int)$room['turn'] === $seat && zjh_requires_showdown($player['stack'] ?? 0, $room['current_bet'], !empty($player['seen']), $room['base']);
    if ($action === 'peek') {
        if (($player['status'] ?? '') !== 'active' || !empty($player['revealed'])) return '你已不能看牌。';
        if ($mustAllIn) return '电影票不足以跟注，请选择 All in 亮牌或弃牌。';
        if (!$player['seen']) { $player['seen'] = true; zjh_log($room, $player['username'] . ' 已看牌'); }
        return '';
    }
    if ((int)$room['turn'] !== $seat || ($player['status'] ?? '') !== 'active' || !empty($player['revealed'])) return '还没轮到你行动。';
    if ($mustAllIn && !in_array($action, ['all_in', 'fold'], true)) return '电影票不足以跟注，请选择 All in 亮牌或弃牌。';
    if ($action === 'all_in') {
        if (!$mustAllIn) return '当前电影票足够跟注，无需 All in。';
        $amount = max(0, (int)$player['stack']);
    ...
    if ($action === 'fold') {
        ...
        $room['current_bet'] += (int)$room['base']; $unit = zjh_action_cost($room['current_bet'], !empty($player['seen']), $room['base']);
        ...
        if ($opponent < 0 || $opponent === $seat || ($room['players'][$opponent]['status'] ?? '') !== 'active' || !empty($room['players'][$opponent]['revealed'])) return '请选择仍可行动的比牌对象。';
...
function zjh_bot_action(&$room, $seat)
{
    ...
    $cost = zjh_action_cost($room['current_bet'], !empty($player['seen']), $room['base']);
...
function zjh_public($room, $uid)
{
    ...
    for ($view=0; $view<$seatCount; $view++) {
        ...
        $show = zjh_may_view_cards($finished, $actual === $viewer, !empty($player['seen']), !empty($player['revealed'])); $cards = [];
        if ($show) foreach (($player['cards'] ?? []) as $card) $cards[] = zjh_card_view($card);
        $publicPlayer = ['username'=>$player['username'], 'status'=>$player['status'], 'cards'=>$cards, 'stack'=>(int)$player['stack'],
            'seen'=>(bool)$player['seen'], 'revealed'=>(bool)($player['revealed'] ?? false),
        ...
        if ($show && count($cards) === 3) $publicPlayer['handName'] = zjh_score_name(zjh_evaluate($player['cards']));
        $players[] = $publicPlayer;
    }
    $turn = (int)($room['turn'] ?? -1); $turnView = $turn < 0 ? -1 : (($turn - $viewer + $seatCount) % $seatCount);
    $me = $room['players'][$viewer]; $unit = zjh_action_cost($room['current_bet'] ?? $room['base'], !empty($me['seen']), $room['base'] ?? 0);
    $canAct = $room['status']==='playing' && $turn===$viewer && ($me['status'] ?? '')==='active' && empty($me['revealed']);
    $mustAllIn = $canAct && (int)$me['stack'] < $unit;
    $compareTargets = [];
    foreach (zjh_unrevealed_active_seats($room['players']) as $actualSeat) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    } elseif (in_array($action, ['peek','call','raise','fold','compare','compare_all','all_in'], true)) {
...
.zjh-table{min-height:760px}.z-dialog.setup{width:min(900px,calc(100% - 30px));padding:0;overflow:hidden;text-align:left;background:linear-gradient(145deg,rgba(36,20,16,.98),rgba(15,10,9,.99));box-shadow:0 30px 90px rgba(0,0,0,.55),inset 0 1px rgba(255,255,255,.04)}.z-lobby-hero{position:relative;display:flex;align-items:center;justify-content:space-between;gap:24px;padding:24px 28px 21px;border-bottom:1px solid var(--z-line);background:radial-gradient(circle at 88% 10%,color-mix(in srgb,var(--bili-primary,var(--z-gold)) 24%,transparent),transparent 32%),linear-gradient(120deg,rgba(120,31,38,.26),rgba(227,180,90,.07))}.z-lobby-hero::after{content:"";position:absolute;right:45px;bottom:-45px;width:150px;height:150px;border:1px solid rgba(255,218,133,.08);border-radius:50%;box-shadow:0 0 0 25px rgba(255,218,133,.025),0 0 0 50px rgba(255,218,133,.018);pointer-events:none}.z-lobby-kicker{display:flex;align-items:center;gap:8px;margin-bottom:7px;color:var(--z-gold);font-size:11px;font-weight:900;letter-spacing:2px}.z-lobby-kicker::before{content:"";width:24px;height:2px;border-radius:2px;background:var(--z-gold)}.zjh-page .z-lobby-hero h2{margin:0 0 6px!important;font-size:30px!important;letter-spacing:1px}.z-lobby-hero p{margin:0;color:#cbbda9;line-height:1.65}.z-lobby-feature-list{position:relative;z-index:1;display:flex;gap:7px;flex-wrap:wrap;justify-content:flex-end}.z-lobby-feature{padding:7px 10px;border:1px solid rgba(255,218,133,.18);border-radius:999px;background:rgba(12,8,7,.42);color:#eadcc7;font-size:11px;white-space:nowrap}.z-lobby-body{padding:22px 28px 26px}.z-lobby-section{margin-bottom:20px}.z-lobby-section-head{display:flex;align-items:end;justify-content:space-between;gap:12px;margin-bottom:10px}.z-lobby-section-title{display:flex;align-items:center;gap:9px;color:#fff5df;font-size:15px;font-weight:900}.z-lobby-step{width:25px;height:25px;border:1px solid rgba(255,218,133,.4);border-radius:8px;display:grid;place-items:center;background:rgba(227,180,90,.1);color:var(--z-gold2);font-size:12px}.z-lobby-hint{color:#a99882;font-size:11px}.z-mode-grid{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin:0}.z-mode-choice{position:relative;min-height:88px;padding:15px 48px 15px 62px;border:1px solid rgba(255,218,133,.16);border-radius:15px;background:linear-gradient(145deg,rgba(55,31,24,.74),rgba(25,16,13,.78));color:#f7ead7;text-align:left;cursor:pointer;transition:border-color .2s,box-shadow .2s,background .2s}.z-mode-choice:hover{border-color:rgba(255,218,133,.42);box-shadow:0 10px 25px rgba(0,0,0,.22)}.z-mode-choice.active{border-color:var(--bili-primary,var(--z-gold));background:linear-gradient(145deg,color-mix(in srgb,var(--bili-primary,var(--z-gold)) 22%,#321d17),rgba(25,16,13,.94));box-shadow:0 0 0 2px color-mix(in srgb,var(--bili-primary,var(--z-gold)) 22%,transparent)}.z-mode-icon{position:absolute;left:15px;top:50%;transform:translateY(-50%);width:34px;height:34px;border-radius:11px;display:grid;place-items:center;background:rgba(227,180,90,.12);color:var(--z-gold2)}.z-mode-icon svg{width:20px;height:20px;fill:none;stroke:currentColor;stroke-width:1.8;stroke-linecap:round;stroke-linejoin:round}.z-mode-choice strong{display:block;margin-bottom:4px;font-size:15px}.z-mode-choice small{color:#bcae9a;font-size:11px}.z-lobby-check{position:absolute;right:14px;top:50%;transform:translateY(-50%);width:24px;height:24px;border:1px solid rgba(255,255,255,.16);border-radius:50%;display:grid;place-items:center;color:transparent;font-weight:1000}.active .z-lobby-check{border-color:#fff;background:#fff;color:#4b261a}.z-number-grid{display:grid;grid-template-columns:repeat(8,1fr);gap:8px}.z-seat-choice{position:relative;min-height:54px;border:1px solid rgba(255,218,133,.15);border-radius:12px;background:rgba(30,19,15,.76);color:#d8cab7;font-size:14px;font-weight:900;cursor:pointer;transition:border-color .2s,background .2s,color .2s,box-shadow .2s}.z-seat-choice:hover{border-color:rgba(255,218,133,.4);color:#fff}.z-seat-choice.active{border-color:var(--bili-primary,var(--z-gold));background:color-mix(in srgb,var(--bili-primary,var(--z-gold)) 18%,#281813);color:#fff;box-shadow:inset 0 -3px var(--bili-primary,var(--z-gold))}.z-seat-choice span{display:block;margin-top:2px;color:#9f8f7a;font-size:9px;font-weight:600}.z-base-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin:0}.z-stake-choice{position:relative;min-height:86px;padding:13px 14px;border:1px solid rgba(255,218,133,.15);border-radius:14px;background:linear-gradient(145deg,rgba(49,29,23,.8),rgba(25,16,13,.84));color:#f5e6cf;text-align:left;cursor:pointer;overflow:hidden;transition:border-color .2s,box-shadow .2s,background .2s}.z-stake-choice::after{content:"";position:absolute;right:-13px;bottom:-17px;width:60px;height:60px;border:8px double rgba(227,180,90,.07);border-radius:50%}.z-stake-choice:hover{border-color:rgba(255,218,133,.42);box-shadow:0 8px 22px rgba(0,0,0,.2)}.z-stake-choice.active{border-color:var(--bili-primary,var(--z-gold));background:linear-gradient(145deg,color-mix(in srgb,var(--bili-primary,var(--z-gold)) 20%,#392019),#1e130f);box-shadow:0 0 0 2px color-mix(in srgb,var(--bili-primary,var(--z-gold)) 20%,transparent)}.z-stake-label{display:block;color:#a9967d;font-size:10px;font-weight:700}.z-stake-choice strong{display:block;margin:3px 0 5px;color:var(--z-gold2);font-size:21px}.z-stake-choice small{color:#c7b8a3;font-size:10px}.z-lobby-footer{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:12px;align-items:center;margin-top:2px;padding:15px;border:1px solid rgba(255,218,133,.2);border-radius:15px;background:linear-gradient(100deg,rgba(227,180,90,.1),rgba(22,14,12,.72))}.z-lobby-summary{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px}.z-summary-item span{display:block;color:#9f907c;font-size:9px}.z-summary-item b{display:block;margin-top:3px;color:#fff1d5;font-size:13px}.z-lobby-submit{min-width:210px;min-height:52px;border:0;border-radius:12px;background:linear-gradient(135deg,#c88a2d,#f0c66c);color:#28170d;font-weight:1000;cursor:pointer;box-shadow:0 10px 24px rgba(198,132,37,.25);transition:filter .2s,box-shadow .2s}.z-lobby-submit:hover{filter:brightness(1.06);box-shadow:0 13px 30px rgba(198,132,37,.34)}.z-mode-choice:focus-visible,.z-seat-choice:focus-visible,.z-stake-choice:focus-visible,.z-lobby-submit:focus-visible{outline:3px solid #fff;outline-offset:3px}.z-waiting-dialog{width:min(720px,calc(100% - 30px));padding:28px;background:linear-gradient(145deg,#2b1712,#130c0a)}.z-waiting-head{display:flex;align-items:center;justify-content:center;gap:14px;margin-bottom:8px}.z-waiting-pulse{width:16px;height:16px;border:3px solid rgba(227,180,90,.24);border-top-color:var(--z-gold2);border-radius:50%;animation:z-spin 1s linear infinite}.z-waiting-meta{display:flex;justify-content:center;gap:8px;flex-wrap:wrap;margin:12px 0 18px}.z-waiting-meta span{padding:6px 10px;border-radius:999px;background:rgba(227,180,90,.09);color:#d7c7b0;font-size:11px}.z-waiting-seats{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:8px;margin:18px 0}.z-wait-seat{min-height:72px;padding:10px 5px;border:1px dashed rgba(255,218,133,.2);border-radius:12px;display:grid;place-items:center;align-content:center;gap:4px;color:#897b68;font-size:10px}.z-wait-seat::before{content:"";width:24px;height:24px;border:2px solid currentColor;border-radius:50%;opacity:.62}.z-wait-seat.occupied{border-style:solid;border-color:color-mix(in srgb,var(--bili-primary,var(--z-gold)) 52%,transparent);background:color-mix(in srgb,var(--bili-primary,var(--z-gold)) 12%,transparent);color:#f2dba9}.z-wait-actions{display:flex;justify-content:center;gap:9px;flex-wrap:wrap}.z-seat.dynamic{width:230px;transform:translate(-50%,-50%);transition:left .25s,top .25s,border-color .2s,box-shadow .2s;z-index:2}.z-seat.dynamic.compact{width:175px;padding:9px}.z-seat.dynamic.dense{width:135px;padding:7px}.z-seat.dynamic.compact .z-card{width:36px;height:51px;font-size:14px}.z-seat.dynamic.dense .z-card{width:28px;height:42px;font-size:11px}.z-seat.dynamic.compact .z-cards,.z-seat.dynamic.dense .z-cards{min-height:44px;gap:4px;margin-top:5px}.z-seat.dynamic.dense .z-name,.z-seat.dynamic.dense .z-stack{font-size:11px}.z-seat.dynamic.dense .z-status{font-size:10px}.z-seat.dynamic.dense .z-clock{width:30px;height:30px;font-size:11px}.z-compare-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:9px;margin:16px 0;max-height:330px;overflow:auto}.z-compare-target{text-align:left;display:flex;justify-content:space-between;align-items:center}.z-compare-all{width:100%;margin-top:4px}.z-result{max-height:320px;overflow:auto}@keyframes z-spin{to{transform:rotate(360deg)}}
@media(max-width:760px){.zjh-page{margin:8px}.zjh-top h1{font-size:21px}.zjh-table{min-height:700px;border-radius:18px}.z-seat.dynamic{width:145px;padding:7px}.z-seat.dynamic.compact,.z-seat.dynamic.dense{width:104px;padding:5px}.z-seat.dynamic .z-card{width:27px!important;height:40px!important;font-size:10px!important}.z-seat.dynamic .z-cards{min-height:40px;gap:3px}.z-seat.dynamic .z-name,.z-seat.dynamic .z-stack{font-size:10px}.z-seat.dynamic .z-status{font-size:9px}.zjh-brand{top:43%}.zjh-brand strong{font-size:21px}.z-actions{bottom:12px;width:96%;justify-content:center;flex-wrap:wrap}.z-btn{min-height:44px;padding:9px 11px}.z-log{display:none}.z-info{grid-template-columns:1fr}.z-stats{grid-template-columns:repeat(2,1fr)}.z-dialog.setup{width:calc(100% - 16px);max-height:calc(100% - 16px);overflow:auto}.z-lobby-hero{align-items:flex-start;flex-direction:column;padding:19px 18px 16px}.zjh-page .z-lobby-hero h2{font-size:24px!important}.z-lobby-feature-list{justify-content:flex-start}.z-lobby-body{padding:16px 18px 20px}.z-mode-grid{grid-template-columns:1fr}.z-number-grid{grid-template-columns:repeat(4,1fr)}.z-base-grid{grid-template-columns:repeat(2,1fr)}.z-seat-choice{min-height:50px}.z-lobby-footer{grid-template-columns:1fr}.z-lobby-summary{grid-template-columns:repeat(3,1fr)}.z-lobby-submit{width:100%;min-width:0}.z-waiting-dialog{padding:20px 15px}.z-waiting-seats{grid-template-columns:repeat(3,minmax(0,1fr))}.z-compare-grid{grid-template-columns:1fr}}
@media(prefers-reduced-motion:reduce){*{scroll-behavior:auto!important;transition:none!important}}
</style>
<style>
.z-overlay.dismissible{cursor:pointer}.z-overlay.dismissible .z-dialog{position:relative;cursor:default}.z-dialog-close{position:absolute;right:12px;top:12px;width:44px;height:44px;border:1px solid var(--z-line);border-radius:12px;display:grid;place-items:center;background:rgba(255,255,255,.05);color:#e8d8c1;font-size:24px;line-height:1;cursor:pointer;z-index:2}.z-dialog-close:hover{background:rgba(255,255,255,.11);color:#fff}.z-dialog-close:focus-visible,.z-finished-actions .z-btn:focus-visible{outline:3px solid var(--z-gold2);outline-offset:2px}.z-result-review-hint{margin:13px 0 0;color:#aa9982;font-size:11px}.z-finished-actions{position:absolute;left:50%;bottom:18px;transform:translateX(-50%);display:flex;justify-content:center;gap:9px;flex-wrap:wrap;width:min(96%,620px);z-index:6}.z-finished-actions .z-btn{min-height:44px;box-shadow:0 8px 24px rgba(0,0,0,.28)}
.z-actions .z-finished-actions{position:static;transform:none;width:auto}
</style>
<div class="zjh-page">
...
let chosenBase=100, chosenMode='match', chosenSeats=3, busy=false, compareOpen=false, dismissedFinishToken='';
...
function dismissFinish(){if(state.game?.status==='finished'){dismissedFinishToken=state.game.token||'finished';render()}}
function showFinish(){dismissedFinishToken='';render()}
function cards(p){
...
function setupHtml(){
 ...
 const matchIcon='<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 7h10M4 12h7M4 17h5"/><path d="m15 13 4 4m0-4-4 4"/></svg>';
 const friendIcon='<svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="9" cy="8" r="3"/><path d="M3 19c0-3 2.5-5 6-5s6 2 6 5"/><path d="M16 7a3 3 0 0 1 0 6m1 2c2.4.5 4 2 4 4"/></svg>';
 const modeButton=(key,title,sub,icon)=>`<button type="button" class="z-mode-choice ${chosenMode===key?'active':''}" aria-pressed="${chosenMode===key}" onclick="chosenMode='${key}';render()"><span class="z-mode-icon">${icon}</span><strong>${title}</strong><small>${sub}</small><span class="z-lobby-check" aria-hidden="true">✓</span></button>`;
 const seatButton=v=>`<button type="button" class="z-seat-choice ${chosenSeats===v?'active':''}" aria-pressed="${chosenSeats===v}" onclick="chosenSeats=${v};render()">${v} 人<span>${v<=4?'轻松局':(v<=7?'热闹局':'大牌桌')}</span></button>`;
 const stakeButton=(v,i)=>`<button type="button" class="z-stake-choice ${chosenBase===v?'active':''}" aria-pressed="${chosenBase===v}" onclick="chosenBase=${v};render()"><span class="z-stake-label">${['新手','进阶','高手','豪华'][i]}底注</span><strong>${money(v)}</strong><small>每位带入 ${money(v*20)} 电影票</small></button>`;
 return `<div class="z-overlay"><div class="z-dialog setup"><header class="z-lobby-hero"><div><div class="z-lobby-kicker">HDV CARD ROOM</div><h2>炸金花匹配大厅</h2><p>选择适合你的牌桌，真人优先匹配，牌局公平结算。</p></div><div class="z-lobby-feature-list"><span class="z-lobby-feature">3–10 人桌</span><span class="z-lobby-feature">30 秒操作</span><span class="z-lobby-feature">超时弃牌</span></div></header>
 <div class="z-lobby-body"><section class="z-lobby-section"><div class="z-lobby-section-head"><div class="z-lobby-section-title"><span class="z-lobby-step">1</span>选择加入方式</div><span class="z-lobby-hint">直接匹配，或邀请好友同桌</span></div><div class="z-mode-grid">${modeButton('match','快速匹配','自动寻找相同人数与底注的牌友',matchIcon)}${modeButton('friend','好友房','创建专属牌桌并分享邀请码',friendIcon)}</div></section>
 <section class="z-lobby-section"><div class="z-lobby-section-head"><div class="z-lobby-section-title"><span class="z-lobby-step">2</span>选择牌桌人数</div><span class="z-lobby-hint">最低 3 人，最多 10 人</span></div><div class="z-number-grid">${[3,4,5,6,7,8,9,10].map(seatButton).join('')}</div></section>
 <section class="z-lobby-section"><div class="z-lobby-section-head"><div class="z-lobby-section-title"><span class="z-lobby-step">3</span>选择牌桌底注</div><span class="z-lobby-hint">按风险偏好选择场次</span></div><div class="z-base-grid">${[100,500,1000,5000].map(stakeButton).join('')}</div></section>
 <footer class="z-lobby-footer"><div class="z-lobby-summary"><div class="z-summary-item"><span>加入方式</span><b>${mode}</b></div><div class="z-summary-item"><span>牌桌配置</span><b>${chosenSeats} 人 · 底注 ${money(chosenBase)}</b></div><div class="z-summary-item"><span>带入 / 全桌上限</span><b>${money(chosenBase*20)} / ${money(chosenBase*20*chosenSeats)}</b></div></div><button class="z-lobby-submit" onclick="send(chosenMode==='match'?'match':'create',{base:chosenBase,seats:chosenSeats})">${chosenMode==='match'?'立即加入匹配':'创建好友房'} →</button></footer></div></div></div>`;
}
...
function render(){
 ...
 if(g.status==='playing')document.getElementById('actions').innerHTML=g.mustAllIn?`<button class="z-btn primary" onclick="send('all_in')" ${busy?'disabled':''}>All in ${money(g.allInAmount)} 并亮牌</button><button class="z-btn danger" onclick="send('fold')" ${busy?'disabled':''}>弃牌</button>`:`<button class="z-btn" onclick="send('peek')" ${!g.canPeek||busy?'disabled':''}>看牌</button><button class="z-btn primary" onclick="send('call')" ${!g.canAct||busy?'disabled':''}>跟注 ${money(g.callCost)}</button><button class="z-btn" onclick="send('raise')" ${!g.canAct||g.raiseCount>=5||busy?'disabled':''}>加注</button><button class="z-btn" onclick="compareOpen=true;render()" ${!g.canAct||!g.compareTargets?.length||busy?'disabled':''}>选择比牌</button><button class="z-btn danger" onclick="send('fold')" ${!g.canAct||busy?'disabled':''}>弃牌</button>`;else if(g.status==='finished')document.getElementById('actions').innerHTML=`<div class="z-finished-actions"><button class="z-btn" onclick="showFinish()">查看结算</button><button class="z-btn primary" onclick="send('rematch')">准备下一局</button><button class="z-btn" onclick="send('leave')">离开牌桌</button></div>`;else document.getElementById('actions').innerHTML='';
 if(g.status==='waiting'){
  compareOpen=false;const waitingSeats=g.players.map((p,i)=>`<div class="z-wait-seat ${p.status!=='empty'?'occupied':''}"><span>${p.status!=='empty'?esc(p.username):'等待玩家'}</span><small>${p.status!=='empty'?'已入座':'空座 '+(i+1)}</small></div>`).join('');document.getElementById('overlay').innerHTML=`<div class="z-overlay"><div class="z-dialog z-waiting-dialog"><div class="z-waiting-head"><span class="z-waiting-pulse" aria-hidden="true"></span><h2>${g.mode==='friend'?'好友房等待入座':'正在匹配牌友'}</h2></div><p>${g.mode==='friend'?'分享邀请码，好友入座后即可开始。':'正在寻找相同人数和底注的真人玩家，请稍候。'}</p><div class="z-waiting-meta"><span>牌桌 #${g.roomId}</span><span>${g.seatCount} 人桌</span><span>底注 ${money(g.base)}</span><span>${g.playerCount}/${g.seatCount} 已入座</span></div>${g.mode==='friend'?`<div class="z-wait-code">${esc(g.invite)}</div>`:''}<div class="z-waiting-seats">${waitingSeats}</div>${g.startError?`<p class="z-result">${esc(g.startError)}</p>`:''}<div class="z-wait-actions">${g.mode==='friend'?`<button class="z-btn primary" onclick="navigator.clipboard.writeText('${esc(g.inviteUrl)}').then(()=>toast('邀请链接已复制'))">复制好友邀请链接</button>`:''}${g.isOwner&&g.playerCount<g.seatCount?'<button class="z-btn primary" onclick="send(\'fill_bots\')">补机器人立即开局</button> ':''}<button class="z-btn" onclick="send('leave')">退出排队</button></div></div></div>`;
 }else if(g.status==='finished'){
  compareOpen=false;const w=g.players[g.winner],showResult=dismissedFinishToken!==(g.token||'finished');document.getElementById('overlay').innerHTML=showResult?`<div class="z-overlay dismissible" onclick="dismissFinish()"><div class="z-dialog" role="dialog" aria-modal="true" aria-label="牌局结算" onclick="event.stopPropagation()"><button type="button" class="z-dialog-close" aria-label="关闭结算，查看牌桌" onclick="dismissFinish()">×</button><h2>${esc(w?w.username:'赢家')} 获胜</h2><p>${esc(g.finishReason)}</p><div class="z-result">${g.players.filter(p=>p.status!=='empty').map(p=>`${esc(p.username)}：${esc(p.handName)} ${p.bot?'':(p.delta>=0?'+':'')+money(p.delta)}`).join('<br>')}</div><p class="z-result-review-hint">点击弹窗外区域可隐藏结算，查看桌上所有玩家的最终手牌。</p><br><button class="z-btn primary" onclick="send('rematch')">准备下一局</button><button class="z-btn" onclick="send('leave')">离开牌桌</button></div></div>`:'';
...
document.addEventListener('keydown',e=>{if(e.key==='Escape'&&state.game?.status==='finished')dismissFinish()});
