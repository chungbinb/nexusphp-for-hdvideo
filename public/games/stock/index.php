...
async function refresh(){try{const r=await fetch('/games/market.php?ajax=quotes',{credentials:'same-origin'}),d=await r.json();if(d.ok){stockState.quotes=d.quotes;stockState.portfolio=d.portfolio;stockState.trades=d.trades;stockState.market=d.market;stockState.wallet=d.wallet;render()}}catch(e){}}
...
el('searchForm').addEventListener('submit',async e=>{e.preventDefault();const value=el('stockSearch').value.trim();if(!/^\d{6}$/.test(value)){show('请输入六位股票代码。',false);return}try{const r=await fetch('/games/market.php?ajax=quote&symbol='+encodeURIComponent(value),{credentials:'same-origin'}),d=await r.json();if(!d.ok)throw new Error(d.message);extraQuotes[d.quote.symbol]=d.quote;selectedSymbol=d.quote.symbol;render()}catch(err){show(err.message||'查询失败。',false)}});
