// Reorder nav stats: row1 = ratio/bonus/invite/claim, row2 = active/up/down/hr.
...
const OPEN = '<div id="top-stats-bar" class="top-stats-bar">';
const NEW_INNER = `
...
fs.writeFileSync(path.join(__dirname, `_backup_functions_navreorder_${stamp}.php`), php, 'utf8');
const s = php.indexOf(OPEN);
if (s < 0) { console.error('ABORT: strip not found'); process.exit(2); }
const innerStart = s + OPEN.length;
const end = php.indexOf('</div>', innerStart);
if (end < 0) { console.error('ABORT: strip close not found'); process.exit(2); }
const region = php.slice(innerStart, end);
// idempotency: already reordered if mybonus appears before text_uploaded
if (region.indexOf('mybonus.php') >= 0 && region.indexOf('mybonus.php') < region.indexOf('text_uploaded')) { console.log('>>> already reordered'); process.exit(0); }
const out = php.slice(0, innerStart) + NEW_INNER + php.slice(end);
console.log('save:', (await putFile(REMOTE, out)).slice(0, 50));
const v = await getFile(REMOTE);
const r2 = v.slice(v.indexOf(OPEN), v.indexOf('</div>', v.indexOf(OPEN)));
console.log((r2.indexOf('mybonus.php') < r2.indexOf('text_uploaded')) ? '>>> VERIFIED: reordered' : '>>> WARNING verify');
process.exit(0);
