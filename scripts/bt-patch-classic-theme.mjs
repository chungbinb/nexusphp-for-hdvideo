// Append the classic-page (details/etc.) personalization override block to modern-refresh.css.
...
const BLOCK = fs.readFileSync(path.join(__dirname, 'qd-classic-theme.css'), 'utf8');
...
fs.writeFileSync(path.join(__dirname, `_backup_modern-refresh-classic_${stamp}.css`), content, 'utf8');
if (content.includes('qd-classic-themed')) { console.log('>>> Already added.'); process.exit(0); }
const patched = content + BLOCK;
console.log('Uploading...');
console.log('resp:', await putFile(REMOTE, patched));
const v = await getFile(REMOTE);
console.log(v.includes('qd-classic-themed') ? '>>> VERIFIED: classic theme override added.' : '>>> WARNING: verify.');
process.exit(0);
