// Shrink the old site's donate.gif banner to ~match the 体验新版 button height. Idempotent + verified.
...
const NEO = '/donate.gif" alt="Make a donation" style="height:34px;width:auto;vertical-align:top;margin-left:8px;margin-top:50px;" /></a>';
...
fs.writeFileSync(path.join(__dirname, `_backup_old_functions_donatesize_${stamp}.php`), content, 'utf8');

if (content.includes('/donate.gif" alt="Make a donation" style="height:34px')) { console.log('\n>>> Already sized. Nothing to do.'); process.exit(0); }
const n = content.split(OLD).length - 1;
if (n !== 1) { console.error(`ABORT: expected 1 donate img, found ${n}.`); process.exit(2); }
const patched = content.replace(OLD, NEO);
console.log('Matched donate img uniquely. delta', patched.length - content.length);
if (!apply) { console.log('\n[DRY RUN] re-run with --apply'); process.exit(0); }
console.log('Uploading...');
console.log('resp:', await putFile(REMOTE, patched));
const v = await getFile(REMOTE);
console.log(v.includes('/donate.gif" alt="Make a donation" style="height:34px') ? '\n>>> VERIFIED: donate banner shrunk.' : '\n>>> WARNING: verification failed.');
process.exit(v.includes('height:34px') ? 0 : 3);
