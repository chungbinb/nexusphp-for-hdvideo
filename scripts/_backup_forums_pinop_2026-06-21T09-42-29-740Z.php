...
if ($action == "viewtopic")
{
	...
	if (isset($page[0]) && $page[0] == "p")
	{
		...
		$res = sql_query("SELECT id FROM posts " . ($threadedReplies ? $rootReplyWhere : $where) . " ORDER BY added $psortSql") or sqlerr(__FILE__, __LINE__);
		$i = 0;
	...
	$res = sql_query("SELECT * FROM posts " . ($threadedReplies ? $rootReplyWhere : $where) . " ORDER BY id $psortSql LIMIT $perpage offset $offset") or sqlerr(__FILE__, __LINE__);
