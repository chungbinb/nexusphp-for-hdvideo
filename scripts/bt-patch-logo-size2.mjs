// Adjust nav-brand logo to 34px (match nav item height) so it centers in the 40px topbar.
...
const OLD = '#mainmenu.menu li.nav-brand img { max-height: 40px !important; max-width: 240px !important; }';
const NEO = '#mainmenu.menu li.nav-brand img { max-height: 34px !important; max-width: 240px !important; vertical-align: middle; }';

console.log('Fetching:', REMOTE);
let content = await getFile(REMOTE);
const stamp = new Date().toISOString().replace(/[:.]/g, '-');
fs.writeFileSync(path.join(__dirname, `_backup_modern-refresh-logo34_${stamp}.css`), content, 'utf8');

if (content.includes('max-height: 34px !important; max-width: 240px')) { console.log('>>> Already 34px.'); process.exit(0); }
const n = content.split(OLD).length - 1;
if (n !== 1) { console.error(`ABORT: expected 1 occurrence of the 40px rule, found ${n}.`); process.exit(2); }
const patched = content.replace(OLD, NEO);
console.log('Uploading (40px -> 34px)...');
console.log('resp:', await putFile(REMOTE, patched));
const v = await getFile(REMOTE);
console.log(v.includes('max-height: 34px !important; max-width: 240px') ? '>>> VERIFIED: logo set to 34px.' : '>>> WARNING: verify.');
process.exit(0);
