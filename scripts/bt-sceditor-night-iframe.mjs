// Dark-theme the SCEditor WYSIWYG iframe content in night mode (via inst.css + theme observer).
...
const ANCHOR = `window.__sceDescr=inst;`;
const INJECT = `window.__sceDescr=inst;
      function qdEdTheme(){var nb=document.documentElement.getAttribute('data-site-theme')==='night'||(document.body&&document.body.classList.contains('theme-night'));try{inst.css(nb?'html,body{background:#0b1422 !important;color:#dbe5f3 !important;}a{color:#5fa7ff !important;}':'');}catch(e){}}
      qdEdTheme();
      try{var qdEtMO=new MutationObserver(qdEdTheme);qdEtMO.observe(document.documentElement,{attributes:true,attributeFilter:['data-site-theme','class']});if(document.body){qdEtMO.observe(document.body,{attributes:true,attributeFilter:['class']});}}catch(e){}`;

let up = await getFile(REMOTE);
const stamp = new Date().toISOString().replace(/[:.]/g, '-');
fs.writeFileSync(path.join(__dirname, `_backup_upload_sceditornight_${stamp}.php`), up, 'utf8');
if (up.includes('function qdEdTheme()')) { console.log('>>> already added'); process.exit(0); }
const n = up.split(ANCHOR).length - 1;
if (n !== 1) { console.error(`ABORT: anchor count ${n}`); process.exit(2); }
up = up.replace(ANCHOR, INJECT);
console.log('save:', (await putFile(REMOTE, up)).slice(0, 50));
const v = await getFile(REMOTE);
console.log(v.includes('function qdEdTheme()') ? '>>> VERIFIED: iframe night theming added.' : '>>> WARNING verify');
process.exit(0);
