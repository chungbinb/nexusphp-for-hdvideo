// Center the top-level nav text items (they were baseline-aligned / pushed low by line-height:24px).
...
const MARK = '/* qd-navtext-vcenter */';
const BLOCK = '\n\n' + MARK + '\n'
  + 'body:not(.inframe) #mainmenu.menu > li { display: inline-flex !important; align-items: center !important; }\n'
  + 'body:not(.inframe) #mainmenu.menu > li > a { vertical-align: middle !important; line-height: 1 !important; align-items: center !important; }\n';

const apply = process.argv.includes('--apply');
console.log('Fetching:', REMOTE);
let content = await getFile(REMOTE);
const stamp = new Date().toISOString().replace(/[:.]/g, '-');
fs.writeFileSync(path.join(__dirname, `_backup_modern-refresh-navtext_${stamp}.css`), content, 'utf8');
...
console.log(v.includes(MARK) ? '>>> VERIFIED: nav text vcenter added.' : '>>> WARNING: verify.');
process.exit(0);
