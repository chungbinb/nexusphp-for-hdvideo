// Enlarge the nav-brand logo to ~40px tall (override existing max-height rules).
...
const MARK = '/* qd-logo-size */';
const BLOCK = '\n\n' + MARK + '\n#mainmenu.menu li.nav-brand img { max-height: 40px !important; max-width: 240px !important; }\n';
...
fs.writeFileSync(path.join(__dirname, `_backup_modern-refresh-logosize_${stamp}.css`), content, 'utf8');

if (content.includes(MARK)) { console.log('\n>>> Already added. Nothing to do.'); process.exit(0); }
const patched = content + BLOCK;
if (!apply) { console.log('\n[DRY RUN] would append logo-size rule.'); process.exit(0); }
console.log('Uploading...');
console.log('resp:', await putFile(REMOTE, patched));
const v = await getFile(REMOTE);
console.log(v.includes(MARK) ? '\n>>> VERIFIED: logo max-height set to 40px.' : '\n>>> WARNING: verification failed.');
process.exit(0);
