// Gate the 帮助-submenu "管理组" item on the staffmem permission (same as staff.php enforces).
...
const OLD = 'print ("<li><a href=\\"staff.php\\">管理组</a></li>");';
const NEO = 'if (user_can(\'staffmem\')) { print ("<li><a href=\\"staff.php\\">管理组</a></li>"); }';
...
fs.writeFileSync(path.join(__dirname, `_backup_functions_staffperm_${stamp}.php`), content, 'utf8');

if (content.includes("if (user_can('staffmem')) { print (\"<li><a href=\\\"staff.php\\\">管理组")) { console.log('\n>>> Already gated.'); process.exit(0); }
const n = content.split(OLD).length - 1;
if (n !== 1) { console.error(`ABORT: expected 1 staff menu item, found ${n}.`); process.exit(2); }
...
console.log(v.includes("if (user_can('staffmem')) { print") ? '\n>>> VERIFIED: 管理组 menu item gated on staffmem.' : '\n>>> WARNING: verification failed.');
process.exit(0);
