// Insert a 签到 (attendance) button in the floating sidebar, between 返回旧版 and 个性化.
...
fs.writeFileSync(path.join(__dirname, `_backup_functions_signin_${stamp}.php`), content, 'utf8');

if (content.includes('href="/attendance.php" title="签到"')) { console.log('\n>>> Already added.'); process.exit(0); }
const n = content.split(OLD).length - 1;
if (n !== 1) { console.error(`ABORT: expected 1 个性化 button, found ${n}.`); process.exit(2); }
...
console.log(v.includes('href="/attendance.php" title="签到"') ? '\n>>> VERIFIED: 签到 button added between 返回旧版 and 个性化.' : '\n>>> WARNING: verification failed.');
process.exit(0);
