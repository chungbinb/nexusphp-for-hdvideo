// Make swatch background !important so it isn't overridden by the theme's button background rule.
...
const OLD = "function setSwatch(el,hex){el.style.background=hex;el.setAttribute('data-color',hex);}";
const NEO = "function setSwatch(el,hex){el.style.setProperty('background',hex,'important');el.setAttribute('data-color',hex);}";

console.log('Fetching:', REMOTE);
let content = await getFile(REMOTE);
const stamp = new Date().toISOString().replace(/[:.]/g, '-');
fs.writeFileSync(path.join(__dirname, `_backup_functions_swatchbg_${stamp}.php`), content, 'utf8');
if (content.includes("el.style.setProperty('background',hex,'important')")) { console.log('>>> Already fixed.'); process.exit(0); }
...
console.log(v.includes("setProperty('background',hex,'important')") ? '>>> VERIFIED: swatch bg now !important.' : '>>> WARNING: verify.');
process.exit(0);
