// Add a visible keyword search bar above the torrents quick-filters block.
...
const ANCHOR = `print('<div class="torrent-quick-filters">' . implode('', $groups) . '</div>');`;
const SEARCHBAR = `print('<form class="qd-torrent-search" method="get" action="?" role="search"><input type="text" name="search" value="' . htmlspecialchars((string)($_GET['search'] ?? '')) . '" placeholder="搜索关键字..." autocomplete="off" /><button type="submit">搜索</button></form>');`;
const NEO = SEARCHBAR + `\n\t` + ANCHOR;

let php = await getFile(REMOTE);
const stamp = new Date().toISOString().replace(/[:.]/g, '-');
fs.writeFileSync(path.join(__dirname, `_backup_torrents_searchbar_${stamp}.php`), php, 'utf8');
if (php.includes('qd-torrent-search')) { console.log('>>> already added'); process.exit(0); }
const n = php.split(ANCHOR).length - 1;
if (n !== 1) { console.error(`ABORT: anchor count ${n}`); process.exit(2); }
php = php.replace(ANCHOR, NEO);
console.log('save:', (await putFile(REMOTE, php)).slice(0, 50));
const v = await getFile(REMOTE);
console.log(v.includes('qd-torrent-search') ? '>>> VERIFIED' : '>>> WARNING verify');
process.exit(0);
